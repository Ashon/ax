...
| **daemon** | `ax.daemon` | 데몬, MCP 서버, 공유 타입과 메시지/작업 프로토콜의 기본 owner입니다. |
| **release** | `ax.release` | 빌드, 테스트, CI/CD, 릴리스와 root build/meta 파일의 기본 owner입니다. |
| **runtime** | `ax.runtime` | 에이전트 런타임, 워크스페이스 라이프사이클, tmux 통합의 기본 owner입니다. |
...
### daemon (`ax.daemon`)
- 데몬, MCP 서버, 공유 타입과 메시지/작업 프로토콜의 기본 owner입니다.
  ...
  fallback ownership:
  - internal/mcpserver/, internal/types/는 cwd가 달라도 ax.daemon이 owner입니다.
  - 메시지 큐, registry, task 모델, daemon wire protocol 변경은 ax.daemon이 우선 owner입니다.
...
### runtime (`ax.runtime`)
- 에이전트 런타임, 워크스페이스 라이프사이클, tmux 통합의 기본 owner입니다.
  ...
  fallback ownership:
  - internal/agent/, internal/tmux/는 cwd가 달라도 ax.runtime가 owner입니다.
  - 런타임/세션/아티팩트 생성 경계에 걸친 glue code는 ax.runtime가 우선 owner입니다.
