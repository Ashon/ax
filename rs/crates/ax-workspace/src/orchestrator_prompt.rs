...
const DEFAULT_PROMPT_N: usize = 6;

#[derive(Debug, thiserror::Error)]
pub enum OrchestratorPromptError {
    #[error("unknown runtime {0:?}")]
    UnknownRuntime(String),
    #[error("remove stale orchestrator prompt {path}: {source}")]
    RemoveStalePrompt {
        path: String,
        #[source]
        source: io::Error,
    },
    #[error("write orchestrator prompt {path}: {source}")]
    WritePrompt {
        path: String,
        #[source]
        source: io::Error,
...
/// Render the scope-specific instruction text for a project orchestrator.
#[must_use]
pub fn orchestrator_prompt(node: &ProjectNode, prefix: &str, parent_name: &str) -> String {
    render_orchestrator_prompt(node, prefix, parent_name, &[])
}

/// Write the generated orchestrator prompt into the runtime's instruction file.
pub fn write_orchestrator_prompt(
    orch_dir: &Path,
    node: &ProjectNode,
    prefix: &str,
    parent_name: &str,
    runtime: &str,
    socket_path: &Path,
) -> Result<(), OrchestratorPromptError> {
    let runtime = Runtime::normalize(runtime)
        .ok_or_else(|| OrchestratorPromptError::UnknownRuntime(runtime.to_owned()))?;
    let memories = prompt_memories_for_orchestrator(prefix, socket_path).unwrap_or_default();
    let content = render_orchestrator_prompt(node, prefix, parent_name, &memories);
    let target = orch_dir.join(runtime.instruction_file());

    for other in SUPPORTED_RUNTIMES {
        if other == runtime {
            continue;
        }
        let other_path = orch_dir.join(other.instruction_file());
        match fs::remove_file(&other_path) {
            Ok(()) => {}
            Err(e) if e.kind() == io::ErrorKind::NotFound => {}
            Err(source) => {
                return Err(OrchestratorPromptError::RemoveStalePrompt {
    ...
    fs::write(&target, content).map_err(|source| OrchestratorPromptError::WritePrompt {
...
fn render_orchestrator_prompt(
    ...
    if is_root {
        ...
        out.push_str("- 여러 프로젝트에 걸친 작업은 서브 오케스트레이터들을 조율합니다.\n");
    ...
    out.push_str("- 여러 워크스페이스에 걸친 작업 → 각 에이전트에게 병렬 위임 후 `read_messages`로 결과 수집\n");
    out.push_str(
    ...
    out.push_str("- 코드 변경, 조사, 검증이 모두 한 owner 범위에 있으면 task를 쪼개기보다 **가장 가까운 owner 1곳**에 먼저 붙이세요.\n");
    out.push_str("- 여러 owner가 필요할 때만 분할하세요. 이 경우 각 owner의 책임 경계와 기대 산출물을 분리해서 보냅니다.\n");
    ...
    out.push_str("- **성공 기준(success criteria)** 을 포함하세요. 완료로 간주할 조건, 기대 동작, 필요한 검증을 명시합니다.\n");
    out.push_str("- **기대 증거(expected evidence)** 를 포함하세요. 예: 수정 파일, 테스트 결과, 재현/검증 절차, 남은 리스크.\n");
    out.push_str("- acceptance criteria가 모호하면 위임하지 마세요. \"무엇이 완료인지\"와 \"어떤 evidence가 있어야 수용하는지\"를 먼저 메시지에 적습니다.\n");
    out.push_str(
    ...
    out.push_str("- follow-up은 다음 경우에만 보냅니다: 약속한 산출물/기한이 지났는데 응답이 없을 때, 보고가 모순될 때, 요구한 증거가 빠졌을 때, 범위 이탈이 보일 때.\n");
    ...
    out.push_str("- 다음 조건이면 stale로 간주할 수 있습니다: 진행 상태가 오래 갱신되지 않음, 응답 없는 대기 상태가 지속됨, pending messages가 쌓임, workspace status와 task 상태가 어긋남, 산출물 약속 시점을 넘김.\n");
    out.push_str("- stale로 보이면 먼저 최근 수신 메시지와 workspace status를 확인해 단순 대기인지 실제 정체인지 구분하세요. 새 정보가 없으면 noisy ping을 보내지 말고 복구 액션으로 바로 넘어가세요.\n");
    ...
    out.push_str("### 용도\n");
    out.push_str("- **Resuming/블로킹 인터랙티브 다이얼로그 해소** — 숫자 선택·yes/no 같은 키 입력이 필요한 프롬프트 통과\n");
    out.push_str("- **리터럴 텍스트 제출** — 임의 문자열을 타이핑 후 Enter로 제출\n");
    out.push_str("- **임의 키 시퀀스 전송** — `C-c` 인터럽트 등 특수키를 포함한 자유 조합\n\n");
    out.push_str("### 사용 예시\n");
    out.push_str("```\n");
    out.push_str("send_keys(workspace=\"ax.foo\", keys=[\"2\", \"Enter\"])    # Resuming 다이얼로그에서 2번 옵션 선택 후 제출\n");
    out.push_str(
    ...
    out.push_str("```\n\n");
    out.push_str("### `interrupt_agent`와의 차이\n");
    out.push_str("- `interrupt_agent`: Escape/C-c 전용 단축 래퍼 (인터럽트만 수행)\n");
    out.push_str("- `send_keys`: 임의 키 시퀀스(특수키 + 리터럴 텍스트) 전송. 블로킹 프롬프트 해소와 리터럴 입력 모두 지원\n");
    ...
    out.push_str("- 다음과 같은 **no-op 상태 메시지에는 회신하지 않습니다**: `\"no new work\"`, `\"nothing to do\"`, `\"대기 중\"`, `\"진행 상황 없음\"`, `\"확인했습니다\"`, `\"thanks\"`, `\"ok\"`.\n");
    out.push_str("- `read_messages`에서 받은 메시지가 단순 상태 공유인지 먼저 판별하세요. 새 작업 요청, 의사결정에 필요한 새 사실, 명시적 질문이 없다면 **무응답으로 종료**합니다.\n");
    out.push_str("- `read_messages`에서 받은 최신 메시지가 이전에 처리한 메시지와 **실질적으로 동일하면** 회신하지 마세요. wording만 조금 바뀐 repeated summary/repeated confirmation도 같은 메시지로 취급합니다.\n");
    out.push_str("- 지금 보내려는 응답이 이전에 이미 보낸 응답과 **실질적으로 동일하면** 다시 보내지 마세요. 같은 no-op/상태/요약을 반복 전송하면 루프가 됩니다.\n");
    out.push_str("- 이미 내가 보낸 결과 요약을 상대가 반복 전달하거나, 내가 이미 알고 있는 상태를 되풀이하는 메시지에도 회신하지 마세요. 같은 상태를 다시 공유하면 루프가 됩니다.\n");
    out.push_str("- `request` 툴의 결과는 도구 반환값으로 받은 것이지 새 메시지가 아닙니다. 그 응답을 받았다고 해서 다시 메시지를 보내지 마세요.\n");
    out.push_str(
    ...
    out.push_str("- 증거 없이 \"끝났다\", \"문제없다\", \"완료했다\"만 말하면 완료로 받지 마세요. 어떤 파일/테스트/검증/결과가 있는지 구체 follow-up을 보내세요.\n");
    out.push_str("- repo/worktree를 건드린 하위 보고라면 `remaining owned dirty files=<none|paths>`가 있는지 확인하세요. 이 항목이 없으면 leftover verification이 빠진 것으로 보고 완료로 수용하지 마세요.\n");
    out.push_str("- 하위가 commit/task slice 하나를 끝낸 것과 더 큰 owner 범위 요청이 수렴한 것은 다를 수 있습니다. 남은 owned dirty files가 있으면 residual scope 또는 후속 unit이 명시될 때만 부분 완료로 다루세요.\n");
    out.push_str("- 보고가 partial, weak, contradictory, no-op이면 그대로 전달하거나 조용히 수용하지 말고, 부족한 항목을 열거한 구체 follow-up 요청을 보내세요.\n");
    out.push_str("- 하위 보고가 **이미 요청한 작업의 완료 결과만 담고 있고** 새 질문, 새 요청, 새 blocker가 없다면 추가 `send_message`를 보내지 마세요. task/result/status만 로컬에서 갱신하고 대화를 종료합니다.\n");
    out.push_str("- **completion-only report** 와 **duplicate completion report** 에는 회신하지 마세요. 이미 완료 처리한 task에 대해 같은 completion 의미의 메시지가 다시 와도 추가 메시지를 보내지 않습니다.\n");
...
fn prompt_memories_for_orchestrator(
    prefix: &str,
    socket_path: &Path,
) -> Result<Vec<Memory>, serde_json::Error> {
    let self_name = orchestrator_name(prefix);
    let scopes = [
...
fn summarize_workspaces(node: &ProjectNode) -> String {
    let mut names: Vec<String> = node.workspaces.iter().map(|ws| ws.name.clone()).collect();
    if !node.children.is_empty() {
        names.push(format!("+{} sub-project(s)", node.children.len()));
    }
    if names.is_empty() {
        "-".to_owned()
    } else {
...
fn project_scope(prefix: &str) -> String {
    let trimmed = prefix.trim();
    if trimmed.is_empty() {
        format!("project:{ROOT_PROJECT_NAME}")
    } else {
        format!("project:{trimmed}")
    }
}

fn workspace_scope(name: &str) -> String {
    format!("workspace:{}", name.trim())
}

fn load_prompt_memories(
    socket_path: &Path,
    scopes: &[String],
    limit: usize,
) -> Result<Vec<Memory>, serde_json::Error> {
    let memory_path = memory_path_for_socket(socket_path);
    let bytes = match fs::read(&memory_path) {
        Ok(bytes) => bytes,
    ...
    let mut memories: Vec<Memory> = serde_json::from_slice(&bytes)?;
    memories.retain(|entry| {
...
fn compare_memory_for_prompt(left: &Memory, right: &Memory) -> Ordering {
    let left_active = left.superseded_at.is_none();
    let right_active = right.superseded_at.is_none();
    right_active
        .cmp(&left_active)
...
fn memory_path_for_socket(socket_path: &Path) -> PathBuf {
    let expanded = expand_socket_path(socket_path);
    expanded
        .parent()
        .map_or_else(|| PathBuf::from(MEMORY_FILE), |dir| dir.join(MEMORY_FILE))
}

fn expand_socket_path(path: &Path) -> PathBuf {
